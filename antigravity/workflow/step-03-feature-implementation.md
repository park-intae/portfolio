...
- [ ] 대형 헤드라인 타이틀: "~ 개발자 박인태 입니다"
- [ ] 3줄 불릿 요약 (강점, 경험, 개발 중심 축)

### 2단계: ABOUT ME Section (`src/components/sections/AboutSection.tsx`)
- [ ] 상단 4개 수치 하이라이트 지표 카드 (95+ Lighthouse 등)
- [ ] 하단 Tech Stack 박스 (Front-End 등 카테고리별 스킬 아이콘/배지)

### 3단계: PROJECTS Section (`src/components/sections/ProjectsSection.tsx`)
- [ ] **초기 상태**: `selectedProject` state의 기본값을 `projects[0]`로 설정
- [ ] **좌측 2x2 카드 그리드**:
...
- [ ] 2컬럼 레이아웃: Left Column (`Experience`), Right Column (`Training`)

---

## ✅ 완료 검증 조건
- [ ] PROJECTS 카드 호버 시 엘리베이션(y축 덤핑 & 그림자) 동작 정상
- [ ] PROJECTS 우측 디테일 패널이 상시 엘리베이션 스타일로 렌더링됨
- [ ] 진입 시 첫 번째 프로젝트(`projects[0]`) 디테일 정보가 기본 표시됨
- [ ] 모바일 해상도에서 프로젝트 선택 시 하단 디테일 패널 스크롤 이동 정상 작동
