    ...
    <section id="about" className="py-20 px-6 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 mb-12">
        {aboutContent.metrics.map((item) => (
          <div
            key={item.id}
            className="glass-panel p-6 rounded-2xl border border-card hover:border-point/40 transition-all group shadow-card"
          >
            ...
            <div key={idx} className="border-b border-card/60 pb-5 last:border-0 last:pb-0">
              ...
              <div className="flex flex-wrap gap-2.5">
                {cat.skills.map((skill, sIdx) => (
                  <span
                    key={sIdx}
                    className="px-3.5 py-1.5 rounded-lg text-xs font-semibold bg-secondary text-main border border-card hover:border-point hover:text-point transition-all"
                  >
                    {skill}
                  </span>
                ))}
